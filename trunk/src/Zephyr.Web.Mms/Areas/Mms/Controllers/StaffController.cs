...
<settings>
    ...
    <orderby>EMPSID</orderby>
        ...
        public dynamic GetPageData(string id)
        {
            ...
            var pQuery = ParamQuery.Instance().Where("EMPSID", id);
            ...
            var result = new
            {
                ...
                //明细数据
                tab0 = new REMP_ChangeService().GetDynamicList(ParamQuery.Instance().Where("EMPSID", id)),
        ...
        public string GetNewRowId(string type, string key, int qty = 1)
        {
            switch (type)
            {
                case "grid0":
                    ...
                    return service0.GetNewKey("CHANID", "maxplus", qty, ParamQuery.Instance().Where("EMPSID", key));
                default:
                    return "";
            }
        }
...
<settings>
    ...
    <where>
        <c column='EMPSID' symbol='equal'></c>
    </where>
</settings>");
...
<settings>
    ...
    <where>
        <c column='CHANID' symbol='equal'></c>      
    </where>
</settings>"));
            tabsWrapper.Add(RequestWrapper.Instance().SetXml(@"
<settings>
