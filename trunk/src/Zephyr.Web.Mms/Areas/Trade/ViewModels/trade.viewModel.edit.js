/**
* 模块名：trade viewModel
* 程序名: trade.viewModel.edit.js
* Copyright(c) 2013-2015 liuhuisheng [ liuhuisheng.xm@gmail.com ] 
**/
var trade = trade || {};
trade.viewModel = trade.viewModel || {};
 
trade.viewModel.edit = function (data) {
    var self = this;
    ...
    this.readonly = ko.computed(function () {return false;});
    ...
    this.addRowClick = function () {
        ...
        //trade.com.selectMaterial(self, { _xml: 'mms.material_dict' });
    };
    this.removeRowClick = function () {
        if (self.readonly()) return;
        self.gridEdit.deleterow();
        //if (self.form.TotalMoney)
        //    trade.com.calcTotalMoneyWhileRemoveRow(self, "Money", "TotalMoney");
    };
    ...
    this.saveClick = function () {
        ...
        var post = {           //传递到后台的数据
            ...
            tabs: [self.gridEdit.getChanges(self.setting.postListFields)]
        };
        if ((self.gridEdit.ended() && com.formValidate()) && (post.form._changed || post.tabs[0]._changed)) {
            com.ajax({
                url: self.urls.edit,
                data: ko.toJSON(post),
                success: function (d) {
    ...
    this.auditClick = function () {
        ...
        trade.com.auditDialog(this.form, function (d) {
            com.ajax({
                type: 'POST',
