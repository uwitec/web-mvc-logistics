...
_8.src = _5 + ($.inArray('?', _5) > -1 ? '&t=' : '?t=') + com.getSetting('theme');//modify by liuhuisheng 解决切换主题时，js缓存导致脚本出错
...
if(/^http|\//i.test(_13["css"])){ //modify by liuhuisheng 解决加载其它目录的js
var url=_13["css"];
}else{
...
if (/^http|\//i.test(_13["js"])) {//modify by liuhuisheng 解决加载其它目录的js
var url=_13["js"];
}else{
...
if(/\.css$/i.test(_1f)){
if (/^http|\//i.test(_1f)) {//modify by liuhuisheng 解决加载其它目录的js
_c(_1f,_20);
}else{
...
easyuifix.easyloader_setting(easyloader, src);//add by liuhuisheng 读取设置
}
}
...
easyuifix.check_boxModel();//add by liuhuisheng 解决boxModel未计算问题for v1.3.4
