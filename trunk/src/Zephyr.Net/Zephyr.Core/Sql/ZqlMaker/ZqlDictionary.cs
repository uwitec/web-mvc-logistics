...
{
    public class ZqlDictionary : Dictionary<DbType, Dictionary<string, IZqlMaker>>
    {
        public IZqlMaker this[DbType dbType,string symbol]
        {
            get
            {
            ...
            set
            {
                if (!this.ContainsKey(dbType) || this[dbType] == null)
                    this[dbType] = new Dictionary<string, IZqlMaker>();

                this[dbType][symbol.ToLower()] = value;
            }
        }

        public Dictionary<string, IZqlMaker> this[DbType dbType]
        {
            get 
            { 
                return base[dbType]; 
            ...
            set
            {
                if (!base.ContainsKey(dbType) || base[dbType] == null)
                    base[dbType] = new Dictionary<string, IZqlMaker>();
