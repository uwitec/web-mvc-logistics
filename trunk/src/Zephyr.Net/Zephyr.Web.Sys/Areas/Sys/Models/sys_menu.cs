...
namespace Zephyr.Web.Sys.Models
{
    public class sys_menuService : ServiceBase<sys_menu>
    {
        protected override bool OnBeforeEditRow(EditEventArgs arg)
        {
            ...
            {
                //以下SQL条件中再包了一层是为了兼容MySql,MySql中删除时不允许从本表检索条件
                var MenuCode = arg.dataNew["_id"].ToString();
                db.Sql(@"
-- 删除角色字段Map
delete from sys_roleMenuColumnMap where menucode = @0;

-- 删除角色按钮Map
delete from sys_roleMenuButtonMap where menucode = @0;

-- 删除角色菜单Map
delete from sys_roleMenuMap       where menucode = @0;

-- 删除菜单按钮Map
delete from sys_menuButtonMap     where menucode = @0;", MenuCode).Execute();
            }
