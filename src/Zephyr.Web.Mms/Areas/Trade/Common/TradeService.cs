...
{
    [Module("Trade")]
    public class TradeService : ServiceBase<ModelBase>
    {
        //审核表单
        ...
        public static void LoginHandler(JObject request) 
        {
            ...
            //var projectName = (new mms_projectService().GetModel(ParamQuery.Instance().Where("ProjectCode", defaultProject)) ?? new mms_project()).ProjectName;
        ...
        public static List<dynamic> GetReceiveSrcBillTypeList()
        {
            ...
            result.Add(new { value = "trailer", text = "内贸订单" });
...
//                        .LoadXmlFile("~/Areas/Mms/Views/Shared/Xml/material_batches.xml")
//                        .ToParamQuery();
...
//                        {
//                            BillNo = row.BillNo,
//                            RowId = int.Parse(NewKey.maxplus(arg.db, tableName, "RowId", ParamQuery.Instance().Where("BillNo", row.BillNo))),
//                            MaterialCode = row.MaterialCode,
//                            Money = row.Money,
//                            Num = handleQty,
