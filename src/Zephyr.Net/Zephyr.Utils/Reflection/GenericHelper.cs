    ...
    public class GenericHelper
    {
        ...
        public static object GetValue(object item, string name)
        {
            if (IsDynamicType(item.GetType()))
                return ReflectionHelper.GetPropertyValueDynamic(item, name);
            
            return ReflectionHelper.GetPropertyValue(item, name);
        }
        ...
        {
            var type = GenericHelper.GetGenericType(list);
            var names = new Dictionary<string,Type>();

            if (GenericHelper.IsDynamicType(type))
            {
                if (list.Count > 0)
                    foreach (var item in GenericHelper.GetDictionaryValues(list[0]))
                        names.Add(item.Key, (item.Value ?? string.Empty).GetType());
            }
            else
            {
                foreach (var p in ReflectionHelper.GetProperties(type))
