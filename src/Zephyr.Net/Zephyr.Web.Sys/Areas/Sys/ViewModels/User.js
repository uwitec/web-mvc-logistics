...
var viewModel = function(){
    ...
    this.saveClick = function () {
        self.gridEdit.ended();
        var changes = self.gridEdit.getChanges(['UserCode', 'UserName','MaterialCode', 'Description', 'IsEnable']);
        if (self.gridEdit.ended() && changes._changed) {
            com.ajax({
                url: '/api/sys/user/edit',
                data: ko.toJSON({ tabs: [changes] }),
                success: function (d) {
    ...
    com.dialog({
        ...
        viewModel: function (w) {
            ...
            this.confirmClick = function () {
                ...
                var data = that.gridEdit.getChanges(['ID', 'UserCode', 'SettingCode', 'SettingName', 'SettingValue', 'Description']);

                for (var i = data.updated.length-1; i >= 0; i--) {
                    var item = data.updated[i];
                    if (item.ID == 0) {
                        ko.utils.arrayRemoveItem(data.updated, item);
                        delete item.ID;
